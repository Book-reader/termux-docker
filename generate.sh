...
case "${TERMUX_PACKAGE_MANAGER}" in
	...
	pacman)
		...
		TERMUX_DOCKER__BOOTSTRAP_VERSION="2025.08.03-r1%2Bpacman-android-7"
...
echo '[*] Linking /etc/passwd to /system/etc/passwd for "docker exec -itu system"...'
# /etc itself must be a folder, not a symbolic link, because when docker runs the container,
# it overwrites the folder itself, and some files, but not the passwd file inside.
